...
let currentPdf = null;
let currentPdfBytes = null; // Track raw bytes for Vault
let scale = 1.5;
let currentPage = 1;
...
// Attachment Editing Context
let stateStack = []; // Stack of { bytes, doc, annos, type, currentFileContext, sessionVaultId }
let currentFileContext = null; // Reference to the file object in activeAnno.files if editing an attachment

// Session Management: Prevent Vault Bloat
let sessionVaultId = null;
let autoSaveTimer = null;
let lastDeletedVaultItem = null; // Memory store for Vault Undo feature
...
function initButtons() {
    try {
        ...
        // Use native addEventListener to bypass Wix stripping inline onclick attributes
        window.handleStartTrip = () => {
            const driverId = driverInput ? driverInput.value.trim() : '';
            if (!driverId) {
                alert("Please enter Driver/Company ID to begin.");
                return;
            }
            activeTrip = {
                driver: driverId,
            ...
            // Create a fresh vault session for this shift
            sessionVaultId = 'session_' + Date.now();

            startOverlay.style.display = 'none';
            showToast(`Shift started for ${driverId}`);

            // Re-render empty state text if no document loaded yet
            if (!currentPdfBytes && window.innerWidth >= 769) {
                showToast("Please tap Import to begin");
            }
        };
        ...
        document.querySelectorAll('.tool-btn').forEach(btn => {
            ...
            btn.onclick = (e) => {
                const ignore = ['btn-save', 'btn-import', 'btn-finish', 'btn-reset', 'btn-start-trip', 'btn-clear-sig', 'btn-save-sig', 'btn-zoom-in', 'btn-zoom-out', 'btn-demo', 'btn-scan-camera', 'btn-upload-file', 'btn-manual-load', 'btn-vault', 'btn-save-load', 'btn-share', 'btn-reset-view', 'btn-mobile-toggle', 'btn-mobile-fab', 'btn-undo'];
        ...
        if (btnManual) btnManual.onclick = () => {
            ...
            const inputs = ['manual-inv', 'manual-po', 'manual-load', 'manual-order', 'manual-truck', 'manual-trailer',
                ...
                'manual-details', 'manual-notes', 'manual-total'];
            inputs.forEach(id => {
        ...
        if (btnSaveForm) {
            btnSaveForm.onclick = async () => {
                try {
                    ...
                    const billedFull = document.getElementById('manual-billed-to')?.value || "CLIENT NAME";
                    // Split billedFull into name and address if possible (e.g. by comma)
                    const billedSplit = billedFull.split(',');
                    const billedTo = billedSplit[0].trim();
                    const billedAddr = billedSplit.length > 1 ? billedSplit.slice(1).join(',').trim() : "Address N/A";

                    const from = document.getElementById('manual-from')?.value || "COLUMBIA TRANSPORT LLC, SC";
                    const to = document.getElementById('manual-to')?.value || "1005 U.S. Street, LA";
                    ...
                    const details = document.getElementById('manual-details')?.value || "Logistics Services";
                    const notes = document.getElementById('manual-notes')?.value || "None";
...
function drawAnnotations() {
    ...
    annotations.forEach((anno, idx) => {
        ...
        if (anno.type !== 'pen') {
            ...
            let scaleVal = scX;
            if (anno.type === 'text') scaleVal = Math.max(1, 1 / scX) * 0.8;
...
// PLACEHOLDER CREDENTIALS - User should update these
const SUPABASE_URL = 'https://your-project-url.supabase.co';
const SUPABASE_KEY = 'your-anon-key';

function initSupabase() {
    if (typeof supabasejs !== 'undefined' && SUPABASE_URL !== 'https://your-project-url.supabase.co') {
        try {
            supabasePortal = supabasejs.createClient(SUPABASE_URL, SUPABASE_KEY);
            console.log("Supabase initialized successfully.");
            const status = document.getElementById('sync-status');
            if (status) {
                status.innerHTML = '<i data-lucide="cloud-check" style="width: 12px;"></i> LIVE';
        ...
        console.warn("Supabase credentials not set. Running in local-only mode.");
        const status = document.getElementById('sync-status');
        if (status) {
            status.style.background = 'rgba(239, 68, 68, 0.1)';
            status.style.color = '#ef4444';
            status.style.borderColor = 'rgba(239, 68, 68, 0.2)';
            status.innerHTML = '<i data-lucide="cloud-off" style="width: 12px;"></i> LOCAL';
            if (window.lucide) lucide.createIcons();
        }
    }
}
...
function updateModalClock() {
    const clock = document.getElementById('modal-current-time');
    if (clock) {
...
setInterval(updateModalClock, 10000); // Update every 10s

async function uploadToSupabase(bytes, fileName, type = 'application/pdf') {
    if (!supabasePortal) return null;
    try {
        ...
        const { data: urlData } = supabasePortal.storage.from('load-packets').getPublicUrl(data.path);
...
window.submitToOffice = async () => {
    ...
    try {
        ...
        // Sync custom name from share menu to activeTrip for vault consistency
        const nameInput = document.getElementById('share-packet-name');
        if (nameInput && nameInput.value.trim()) {
            activeTrip.inv = nameInput.value.trim();
        }

        // FOOLPROOF: Bake the packet FIRST so all annotations are included!
        const blob = await bakeFullPacket();
        if (!blob) throw new Error("Could not bake final document.");

        // Update currentPdfBytes with the baked version for vault/cloud
        ...
        // Always save to vault as fallback - IMMEDIATE to prevent race conditions
        saveToVault(activeTrip.inv, true);

        if (supabasePortal) {
            const { error } = await supabasePortal.from('loads').insert([loadData]);
            if (error) throw error;
        ...
        alert(supabasePortal ? "Packets successfully submitted to OFFICE CLOUD!" : "Saved to Weekly Vault (Local Only).");
...
if (btnRefreshAdmin) btnRefreshAdmin.onclick = () => refreshAdminList();
...
// --- APP INITIALIZATION ---
console.log("App script reached the end. Initializing buttons directly...");
if (typeof initButtons === 'function') {
    initButtons();
} else {
    console.error("initButtons function is not defined at initialization time!");
}
